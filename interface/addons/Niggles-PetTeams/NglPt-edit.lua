...
local SPECIES_RANGES  =
{
  {824,               833}, -- Species using the standard pet family icons
  {  1,               823},
  {834, L.MAX_PET_SPECIES}
};
...
local function petTeamEditIconOnReceiveDrag(self)
  ...
  if (iconPathId ~= nil) then
    petTeamEditInfo.iconPathId = iconPathId;
    self.normal:SetTexture(L.petTeamIconGetTexture(iconPathId));
  end
...
local function opponentDropDownButtonOnClick(button, userData)
  ...
  if (oldOpponentId > 1) then
    ...
    for idx = 1, 4 do
      ...
      if (iconId ~= nil) then
        ...
        if (iconId <= L.MAX_PET_SPECIES) then
          iconId = teamIcons[iconSpeciesMapping[iconId]];
        end
  ...
  if (petTeamEditInfo.iconPathId == 0) then
    ...
    if ((iconId > 0) and (iconId <= L.MAX_PET_SPECIES)) then
...
local function opponentDropDownOnClick(self, mouseButton)
  ...
  if ((L.searchDropDown:GetParent() ~= self) or
    ...
    L.searchDropDown:SetCallbacks(opponentDropDownListUpdate,
      opponentDropDownButtonOnClick, opponentDropDownOnTextChanged,
      petTeamEditInfo.opponentId);
...
local function petTeamEditCreate()
  ...
  frame = CreateFrame("Frame", nil, NigglesPetTeams,
  ...
  child:SetBackdropBorderColor(TOOLTIP_DEFAULT_COLOR.r,
  ...
  child = frame.opponent;
  child:SetScript("OnClick", opponentDropDownOnClick);
  child.label:SetText(L["Opponent"]);

  -- Initialise the import button
  child = frame.import;
  child:SetText(L["Import"]);
...
--
function L.petTeamEdit(teamInfo)
  -- Local Variables
  local _;
  local abilityId = {};
  local petGuid;
  ...
  L.petTeamCopy(teamInfo, petTeamEditInfo);
  ...
  if (teamInfo == nil) then
    ...
    if (targetIconId ~= nil) then
      ...
      if (targetIconId <= L.MAX_PET_SPECIES) then
  ...
  L.petTeamEditUpdate();
  frame.import:SetShown(teamInfo == nil);
...
function L.petTeamIconGetTexture(iconPathId)
  ...
  if (next(teamIcons) == nil) then
    ...
    for _, range in ipairs(SPECIES_RANGES) do
  ...
  if (type(iconPathId) == "number") then
    if (iconPathId <= L.MAX_PET_SPECIES) then
      _, texture = cpjGetPetInfoBySpeciesID(iconPathId);
    else
      texture = L.opponentGetIconPath(iconPathId);
    end
