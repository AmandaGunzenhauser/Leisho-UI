...
NigglesPetTeamsMenuClass =
{
  ...
  Show = function(self, menuData, anchor, ...)
    ...
    for itemIdx, itemInfo in ipairs(items) do
      ...
      if ((itemType ~= nil) and (isVisible)) then
        ...
        if (bit.band(buttonFlags, MENU_FLAG_LABEL) > 0) then
          ...
          buttonWidth = button.text:GetWidth()+MENU_EXPAND_WIDTH;
        end
        ...
        -- If either check or expand is visible...
        if (bit.band(buttonFlags, MENU_FLAG_CHECK) > 0) then
          -- ...adjust the computed width of the button
    ...
    if (numButtons > 0) then
      ...
      -- Set the menu's backdrop, based on its style
      backdrop = self.backdropStyles[self.style];
      frame:SetBackdrop(backdrop);
      if (backdrop.bgColor ~= nil) then
        frame:SetBackdropColor(backdrop.bgColor.r, backdrop.bgColor.g,
          backdrop.bgColor.b, backdrop.bgColor.a);
      end
    ...
    -- Check if this is the progenitor
