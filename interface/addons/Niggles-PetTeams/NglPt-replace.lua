...
-- This function MUST be followed by a call to 'petFiltersRestore', preferably
-- before the calling function returns.
--
local function petFiltersClear()
  -- Local Variables
  ...
  for flag, value in pairs(petFlagSettings) do
    petFilters.flags[flag] = not cpj.IsFlagFiltered(flag);
    cpj.SetFlagFilter(flag, value);
  end
  for idx = 1, cpj.GetNumPetTypes() do
    petFilters.types[idx] = not cpj.IsPetTypeFiltered(idx);
  end
  for idx = 1, cpj.GetNumPetSources() do
    petFilters.sources[idx] = not cpj.IsPetSourceFiltered(idx);
  end
  cpj.AddAllPetTypesFilter();
  cpj.AddAllPetSourcesFilter();
...
--
local function petFiltersRestore()
  -- Local Variables
  local cpj = C_PetJournal;
  ...
  for idx, value in ipairs(petFilters.sources) do
    cpj.SetPetSourceFilter(idx, value);
  end
...
-- GUID.
--
function L.petReplaceAll(importPets, teamInfo)
  -- Local Variables
  local _;
  local canBattle;
  local cpjGetPetInfoByIndex = C_PetJournal.GetPetInfoByIndex;
    ...
    if ((isOwned) and (not isRevoked) and (canBattle)) then
      ...
      for petIdx = 1, L.MAX_ACTIVE_PETS do
        ...
        if ((current.speciesId ~= 0) and
            ...
            (repSpeed     == current.speed    ) and
            (repQuality   == current.quality  )) then
