...
local function teamDropDownButtonOnClick(button, userData)
  ...
  if (teamInfo ~= nil) then
    ...
    frame.team:SetText(teamInfo.name ~= "" and
      teamInfo.name or L.petTeamOpponentById(teamInfo.opponentId));
...
local function teamDropDownOnClick(self, mouseButton)
  ...
  if ((L.searchDropDown:GetParent() ~= self) or
    ...
    L.searchDropDown:SetCallbacks(teamDropDownListUpdate,
      teamDropDownButtonOnClick, teamDropDownOnTextChanged,
...
function L.petTeamStrategyShow(parent, layoutName, teamInfo)
  ...
  if (layoutName == "preview") then
    -- Hide the drop down button for selecting pet teams
    frame.team:Hide();
    frame.edit:Hide();
  else
    ...
    frame.team:Show();
    frame.team:SetText(teamInfo == nil and L["Select"] or
      (teamInfo.name ~= "" and teamInfo.name or
        L.petTeamOpponentById(teamInfo.opponentId)));
  end
  frame.teamInfo = teamInfo;

  -- Hide the search drop down, if it is being displayed for this frame
  if (L.searchDropDown:GetParent() == frame) then
    L.searchDropDown:Hide();
  end
