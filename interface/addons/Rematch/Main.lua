...
rematch.LEGION = select(4,GetBuildInfo())>=70000
-- nLEGION is a flag for restricting rematch to the either journal or standalone mode
-- it's separate from above flag because this one will (hopefully) be turned off when scrollframe bugs are fixed
rematch.nLEGION = false -- this appears fixed in the latest build (21846)
rematch.jLEGION = nil -- whether the addon runs as a journal if nLEGION true (defined at PLAYER_LOGIN)
...
function rematch:PLAYER_LOGIN()
	...
	-- for locking down which mode the addon runs via the preferred window mode
	if rematch.nLEGION then
		rematch.jLEGION = settings.PreferredMode==3 and true
		settings.UseDefaultJournal = not rematch.jLEGION
	end
	...
	if IsAddOnLoaded("miirGUI") then
		...
		rematch:SetColorTexture(rematch.PetCard.Back.Middle.LoreBG,1,0.82,0.5)
	end
...
-- temporary for legion where SetTexture(r,g,b) is replaced with SetColorTexture(r,g,b)
function rematch:SetColorTexture(texture,r,g,b,a)
...
function rematch:LegionChoiceTooltip(fromDefault)
	local note = "The Legion client has major bugs with moving scrollframes between the pet journal and standalone frames.\n\nUntil this is fixed, you can only choose whether to use Rematch as a standalone window \124cffffffffOR\124r integrated with the journal. \124cffffffffNOT\124r both. The choice is decided by the Preferred Window Mode in Options.\n\nChoosing a new Preferred Window Mode will confirm the choice and then Reload the UI.\n\nAs soon as these bugs are fixed both modes will be usable interchangably again."
	if fromDefault then
		note = note.."\n\nTo get to options:\nEnter \124cffffffff/rematch\124r into chat and then click the Options tab of the standalone window."
	end
	rematch.ShowTooltip(self,"Rematch Legion Beta Note",note,"cursor")
end
