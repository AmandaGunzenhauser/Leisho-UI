...
panel.widgets = { "Header", "Text", "CheckButton", "RadioButton", "Growth", "CustomScale", "LegionChoice" }
...
rematch:InitModule(function()
	...
	-- this whole if/else can go when legion crash bug fixed
	if rematch.nLEGION then
		for i=#panel.opts,1,-1 do
			if panel.opts[i][2]=="UseDefaultJournal" then
				tremove(panel.opts,i)
			end
		end
	else
		for i=#panel.opts,1,-1 do
			if panel.opts[i][2]=="LegionChoice" then
				tremove(panel.opts,i)
			end
		end
	end
...
function panel:RadioButtonOnClick()
	local opt = panel.opts[self:GetParent().index]
	if opt[2]=="PreferredMode" and rematch.nLEGION then
		local targetMode = opt[5]
		if (settings.PreferredMode==3 and targetMode~=3) or (settings.PreferredMode<3 and targetMode==3) then
			local dialog = rematch:ShowDialog("LegionChoice",300,158,"Legion Beta Preferred Mode","Change mode and Reload the UI?",YES,ReloadUI,NO,function() settings.PreferredMode=rematch.Dialog:GetContext("oldPreferredMode") panel:Update() end)
			dialog:ShowText(format("This will make Rematch only work in the %s%s\124r\ mode for the Legion beta until a future update.",rematch.hexWhite,targetMode==3 and "Pet Journal" or "Standalone"),240,64,"TOP",0,-32)
			dialog:SetContext("oldPreferredMode",settings.PreferredMode)
		end
	end
