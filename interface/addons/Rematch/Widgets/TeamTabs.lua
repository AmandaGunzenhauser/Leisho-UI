...
-- temporary until legion launches; if icon is a number it will SetToFileData, until SetTexture works in Legion
function rematch:SetLegionTexture(texture,icon)
	if type(icon)=="number" and not rematch.LEGION then
		texture:SetToFileData(icon)
	else
...
function panel:ShowTabEditDialog(index)
	...
	dialog.Slot:Show()
	rematch:SetLegionTexture(dialog.Slot.Icon,icon)
	...
	for i=1,#buttons do
		...
		if ( index <= numData) then
			for j=1,7 do
				...
				if iconID then
					rematch:SetLegionTexture(subbutton.Texture,iconID)
...
function panel:IconOnClick()
	...
	if iconID then
		...
		rematch:SetLegionTexture(dialog.Slot.Icon,iconID)
		panel:UpdateTabIconPickerList()
	end
end
...
function panel:ShowTabDeleteDialog(index)
	...
	dialog.Slot:Show()
	rematch:SetLegionTexture(dialog.Slot.Icon,groups[index][2])
end
...
function panel:TabButtonUpdate(index)
	...
	if index>0 and index<=numGroups then
		...
		rematch:SetLegionTexture(button.Icon,group[2])
