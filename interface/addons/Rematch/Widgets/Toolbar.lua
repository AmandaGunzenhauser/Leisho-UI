...
rematch:InitModule(function()
	...
	-- WoW patch 6.2 has a bug where logging out/in will not show proper cooldowns spinners on existing buffs
	-- with SetCooldown. To see this in action: hearthstone, wait a few mins, log out and log back in and look
	-- at its cooldown. It will be at the "just cast" point of animation and stay there until CD is up.
	-- To get around this, if either pet treat is up upon login, then blizzBugFixLoginTime will be the GetTime()
	-- of this login, and any cooldowns that go before this time will be adjusted to start at this time.
	-- (for Legion: test if this is still necessary; hopefully blizzard fixes this!)
	if not rematch.LEGION then
		C_Timer.After(0.1,function()
			if UnitBuff("player",GetItemSpell("item:98114") or "") or UnitBuff("player",GetItemSpell("item:98112") or "") then
				blizzBugFixLoginTime = GetTime()
			end
		end)
	end
...
function toolbar:PetCountOnEnter()
	...
	rematch.ShowTooltip(self.PetCount,BATTLE_PETS_TOTAL_PETS,format(L["%s\n\nPets At Max Level: %s%d\124r\nPets Not Collected: %s%d\124r\n\n%s Click to display more about your collection."],BATTLE_PETS_TOTAL_PETS_TOOLTIP,rematch.hexWhite,at25,rematch.hexWhite,missing,rematch.LMB))
end
